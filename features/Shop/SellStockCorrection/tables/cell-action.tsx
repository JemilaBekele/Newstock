...
import { deleteSellStockCorrection } from '@/service/SellStockCorrection';
...
> = ({ data }) => {
  ...
  const onConfirm = async () => {
    if (!data?.id) {
      toast.error('Stock correction ID is missing. Cannot delete.');
      return;
    }

    setLoading(true);
    try {
      await deleteSellStockCorrection(data.id);
      setOpen(false);
      router.refresh();
